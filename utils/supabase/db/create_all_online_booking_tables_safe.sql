-- =====================================================
-- SISTEMA COMPLETO PRENOTAZIONI ONLINE (VERSIONE SICURA)
-- =====================================================
-- Questo script crea tutte le tabelle necessarie per il sistema
-- di prenotazioni online. Gestisce il caso in cui trigger e funzioni esistono già.
-- =====================================================
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_booking_settings_salon_id') THEN
        CREATE INDEX idx_online_booking_settings_salon_id ON online_booking_settings(salon_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_booking_settings_enabled') THEN
        CREATE INDEX idx_online_booking_settings_enabled ON online_booking_settings(enabled) WHERE enabled = true;
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_bookings_customer_email') THEN
        CREATE INDEX idx_online_bookings_customer_email ON online_bookings(customer_email);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_bookings_service') THEN
        CREATE INDEX idx_online_bookings_service ON online_bookings(service_id);
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_extra_schedules_salon_member') THEN
        CREATE INDEX idx_extra_schedules_salon_member ON extra_schedules(salon_id, team_member_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_extra_schedules_date') THEN
        CREATE INDEX idx_extra_schedules_date ON extra_schedules(date);
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_booking_logs_order_id') THEN
        CREATE INDEX idx_online_booking_logs_order_id ON online_booking_logs(order_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_booking_logs_salon_id') THEN
        CREATE INDEX idx_online_booking_logs_salon_id ON online_booking_logs(salon_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_booking_logs_status') THEN
        CREATE INDEX idx_online_booking_logs_status ON online_booking_logs(booking_status);
    END IF;
END $$;

-- =====================================================
-- FUNZIONI E TRIGGER (SOLO SE NON ESISTONO)
-- =====================================================
...
-- Trigger per tutte le tabelle (solo se non esistono)
DO $$
BEGIN
    -- Trigger per online_booking_settings
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_online_booking_settings_updated_at') THEN
    ...
    -- Trigger per extra_schedules
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_extra_schedules_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_online_booking_logs_updated_at') THEN
