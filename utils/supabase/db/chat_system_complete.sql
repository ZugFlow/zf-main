...
-- Policy per chat_groups: gli utenti possono vedere solo i gruppi di cui sono membri
...
-- Policy per aggiornare gruppi: solo admin del gruppo
...
-- Policy per eliminare gruppi: solo admin del gruppo
CREATE POLICY "Group admins can delete groups" ON chat_groups
    FOR DELETE USING (
        id IN (
            SELECT group_id FROM chat_group_members 
            WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Users can view members of groups they belong to" ON chat_group_members
    FOR SELECT USING (
        group_id IN (
            SELECT group_id FROM chat_group_members 
            WHERE user_id = auth.uid()
...
CREATE POLICY "Group admins can manage members" ON chat_group_members
    FOR ALL USING (
        group_id IN (
            SELECT group_id FROM chat_group_members 
            WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Users can view reactions in groups they belong to" ON chat_message_reactions
    FOR SELECT USING (
        message_id IN (
...
CREATE POLICY "Group members can manage their own reactions" ON chat_message_reactions
    FOR ALL USING (
        auth.uid() IS NOT NULL AND
        user_id = auth.uid() AND
        message_id IN (
...
CREATE POLICY "Users can manage their own read status" ON chat_message_reads
