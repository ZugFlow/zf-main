-- =====================================================
-- CREAZIONE TABELLA WORK_HOURS - VERSIONE CORRETTA
-- Compatibile con la struttura database esistente
-- =====================================================
...
COMMENT ON COLUMN work_hours.salon_id IS 'ID del salone (riferimento a team.salon_id)';
...
VALUES (
  (SELECT id FROM team WHERE user_id = 'user-uuid' LIMIT 1), -- Sostituisci con user_id reale
  (SELECT salon_id FROM team WHERE user_id = 'user-uuid' LIMIT 1), -- Sostituisci con user_id reale
  CURRENT_DATE,
  '09:00:00',
  '17:00:00',
  'Giornata normale'
...
SELECT * FROM get_member_work_hours(
  (SELECT id FROM team WHERE user_id = 'user-uuid' LIMIT 1), -- Sostituisci con user_id reale
  CURRENT_DATE - INTERVAL '30 days',
  CURRENT_DATE
);
