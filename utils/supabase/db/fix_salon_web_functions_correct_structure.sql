-- =====================================================
-- FUNZIONI SALON WEB - VERSIONE CORRETTA PER STRUTTURA REALE DB
-- =====================================================
...
-- Funzione per ottenere le impostazioni web di un salone
CREATE OR REPLACE FUNCTION get_salon_web_settings(
    p_user_id UUID,
    p_salon_id UUID
)
RETURNS JSON AS $$
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN json_build_object(
            'success', true,
            'data', NULL,
            'message', 'Nessuna impostazione web trovata per questo salone'
        );
    END IF;
    
    RETURN json_build_object(
        'success', true,
        'data', row_to_json(web_settings),
        'message', 'Impostazioni web recuperate con successo'
    );
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN json_build_object(
            'success', false, 
            'error', 'Errore durante il recupero delle impostazioni: ' || SQLERRM
        );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
COMMENT ON FUNCTION enable_salon_web_page IS 'Abilita la pagina web di un salone (versione corretta per struttura reale DB)';
COMMENT ON FUNCTION disable_salon_web_page IS 'Disabilita la pagina web di un salone';
COMMENT ON FUNCTION get_salon_web_settings IS 'Recupera le impostazioni web di un salone';
