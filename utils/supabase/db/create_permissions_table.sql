...
CREATE TABLE IF NOT EXISTS permissions (
    ...
    -- Team member requesting permission
    member_id UUID NOT NULL REFERENCES team(id) ON DELETE CASCADE,
    
    -- Salon context for proper data isolation
    ...
    -- Ensure salon_id matches the member's salon
    CONSTRAINT valid_salon_member CHECK (
        salon_id = (SELECT salon_id FROM team WHERE id = member_id)
    )
);
...
COMMENT ON COLUMN permissions.member_id IS 'Reference to the team member requesting permission';
COMMENT ON COLUMN permissions.salon_id IS 'Reference to the salon for data isolation';
