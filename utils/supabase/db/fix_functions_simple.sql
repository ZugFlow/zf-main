-- =====================================================
-- FIX POSTGRESQL ERROR 42P13 - SEMPLIFICATO
-- =====================================================
-- Script da eseguire direttamente nel Supabase Dashboard SQL Editor
-- Risolve l'errore "cannot remove parameter defaults from existing function"

-- 1. ELIMINA LE FUNZIONI PROBLEMATICHE
-- =====================================================

-- Funzioni per gestione galleria
DROP FUNCTION IF EXISTS add_gallery_image(UUID, VARCHAR, TEXT, TEXT, VARCHAR, VARCHAR, INTEGER);
DROP FUNCTION IF EXISTS update_gallery_image(UUID, VARCHAR, TEXT, TEXT, VARCHAR, VARCHAR, INTEGER, BOOLEAN);
DROP FUNCTION IF EXISTS delete_gallery_image(UUID);

-- Funzioni per gestione testimonial
DROP FUNCTION IF EXISTS add_testimonial(UUID, VARCHAR, VARCHAR, INTEGER, TEXT, VARCHAR);
DROP FUNCTION IF EXISTS approve_testimonial(UUID, BOOLEAN, BOOLEAN);
DROP FUNCTION IF EXISTS delete_testimonial(UUID);

-- Funzioni per analytics
DROP FUNCTION IF EXISTS record_page_visit(UUID, VARCHAR, INET, TEXT, VARCHAR, VARCHAR);
DROP FUNCTION IF EXISTS get_salon_web_analytics(UUID, DATE, DATE);

-- Funzioni per prenotazioni
DROP FUNCTION IF EXISTS confirm_web_booking(UUID, TEXT);
DROP FUNCTION IF EXISTS cancel_web_booking(UUID, TEXT);

-- Funzioni per messaggi di contatto
DROP FUNCTION IF EXISTS mark_contact_message_read(UUID, BOOLEAN);
DROP FUNCTION IF EXISTS mark_contact_message_replied(UUID, BOOLEAN);

-- Funzioni per impostazioni avanzate
DROP FUNCTION IF EXISTS update_seo_settings(UUID, VARCHAR, TEXT, TEXT, TEXT, TEXT, VARCHAR);
DROP FUNCTION IF EXISTS update_design_settings(UUID, VARCHAR, VARCHAR, VARCHAR, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS update_web_features(UUID, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN);

-- Funzioni per backup e ripristino
DROP FUNCTION IF EXISTS export_salon_web_data(UUID);
DROP FUNCTION IF EXISTS cleanup_old_web_data(UUID, INTEGER);

-- Funzioni per abilitare/disabilitare pagina web
DROP FUNCTION IF EXISTS enable_salon_web_page(UUID, UUID);
DROP FUNCTION IF EXISTS disable_salon_web_page(UUID, UUID);

-- 2. RICREA LE FUNZIONI PRINCIPALI
-- =====================================================
...
BEGIN
    ...
    IF p_title IS NULL OR p_image_url IS NULL THEN
        RETURN json_build_object('success', false, 'error', 'Titolo e URL immagine sono obbligatori');
    END IF;

    -- Inserisci la nuova immagine
    INSERT INTO salon_galleries (salon_id, title, description, image_url, image_alt, category, sort_order)
    VALUES (p_salon_id, p_title, p_description, p_image_url, p_image_alt, p_category, p_sort_order)
    RETURNING id INTO v_gallery_id;

    RETURN json_build_object('success', true, 'gallery_id', v_gallery_id, 'message', 'Immagine aggiunta con successo');
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN json_build_object('success', false, 'error', 'Immagine non trovata');
    END IF;

    RETURN json_build_object('success', true, 'message', 'Immagine aggiornata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    RETURN json_build_object('success', true, 'message', 'Immagine eliminata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    IF p_rating < 1 OR p_rating > 5 THEN
        RETURN json_build_object('success', false, 'error', 'Il rating deve essere tra 1 e 5');
    END IF;
    ...
    IF p_client_name IS NULL OR p_comment IS NULL THEN
        RETURN json_build_object('success', false, 'error', 'Nome cliente e commento sono obbligatori');
    END IF;

    -- Inserisci il testimonial (non approvato di default)
    INSERT INTO salon_testimonials (salon_id, client_name, client_email, rating, comment, service_name, is_approved)
    VALUES (p_salon_id, p_client_name, p_client_email, p_rating, p_comment, p_service_name, false)
    RETURNING id INTO v_testimonial_id;

    RETURN json_build_object('success', true, 'testimonial_id', v_testimonial_id, 'message', 'Testimonial aggiunto con successo (in attesa di approvazione)');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    SET is_approved = p_is_approved, is_featured = p_is_featured, updated_at = NOW()
    WHERE id = p_testimonial_id;

    IF NOT FOUND THEN
        RETURN json_build_object('success', false, 'error', 'Testimonial non trovato');
    END IF;

    RETURN json_build_object('success', true, 'message', 
        CASE WHEN p_is_approved THEN 'Testimonial approvato con successo' ELSE 'Testimonial disapprovato con successo' END);
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    RETURN json_build_object('success', true, 'message', 'Testimonial eliminato con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM salon_web_settings WHERE salon_id = p_salon_id AND web_enabled = true) THEN
        RETURN json_build_object('success', false, 'error', 'Salone non trovato o pagina web non abilitata');
    END IF;

    -- Inserisci la visita
    INSERT INTO web_analytics (salon_id, page_url, visitor_ip, user_agent, referrer, session_id, visit_date, visit_time)
    VALUES (p_salon_id, p_page_url, p_visitor_ip, p_user_agent, p_referrer, p_session_id, CURRENT_DATE, CURRENT_TIME)
    RETURNING id INTO v_analytics_id;

    RETURN json_build_object('success', true, 'analytics_id', v_analytics_id, 'message', 'Visita registrata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    WITH analytics_stats AS (
        ...
        FROM web_analytics 
        WHERE salon_id = p_salon_id AND visit_date BETWEEN v_start_date AND v_end_date
    ),
    booking_stats AS (
        SELECT 
        ...
        FROM web_bookings 
        WHERE salon_id = p_salon_id AND appointment_date BETWEEN v_start_date AND v_end_date
    ),
    contact_stats AS (
        SELECT 
        ...
        WHERE salon_id = p_salon_id AND created_at::date BETWEEN v_start_date AND v_end_date
    )
...
BEGIN
    UPDATE web_bookings 
    SET status = 'confirmed', notes = COALESCE(p_admin_notes, notes), updated_at = NOW()
    WHERE id = p_booking_id;

    IF NOT FOUND THEN
        RETURN json_build_object('success', false, 'error', 'Prenotazione non trovata');
    END IF;

    RETURN json_build_object('success', true, 'message', 'Prenotazione confermata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    UPDATE web_bookings 
    SET status = 'cancelled', notes = COALESCE(p_cancellation_reason, notes), updated_at = NOW()
    WHERE id = p_booking_id;
    ...
    RETURN json_build_object('success', true, 'message', 'Prenotazione cancellata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    SET is_read = p_is_read, updated_at = NOW()
    WHERE id = p_message_id;

    IF NOT FOUND THEN
        RETURN json_build_object('success', false, 'error', 'Messaggio non trovato');
    END IF;

    RETURN json_build_object('success', true, 'message', 
        CASE WHEN p_is_read THEN 'Messaggio segnato come letto' ELSE 'Messaggio segnato come non letto' END);
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    UPDATE web_contact_messages 
    SET is_replied = p_is_replied, updated_at = NOW()
    WHERE id = p_message_id;
    ...
    RETURN json_build_object('success', true, 'message', 
        CASE WHEN p_is_replied THEN 'Messaggio segnato come risposto' ELSE 'Messaggio segnato come non risposto' END);
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    RETURN json_build_object('success', true, 'message', 'Impostazioni SEO aggiornate con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    RETURN json_build_object('success', true, 'message', 'Impostazioni di design aggiornate con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    RETURN json_build_object('success', true, 'message', 'Funzionalità web aggiornate con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'galleries', COALESCE((SELECT json_agg(row_to_json(sg.*)) FROM salon_galleries sg WHERE sg.salon_id = p_salon_id), '[]'::json),
        'testimonials', COALESCE((SELECT json_agg(row_to_json(st.*)) FROM salon_testimonials st WHERE st.salon_id = p_salon_id), '[]'::json),
        'bookings', COALESCE((SELECT json_agg(row_to_json(wb.*)) FROM web_bookings wb WHERE wb.salon_id = p_salon_id), '[]'::json),
        'contact_messages', COALESCE((SELECT json_agg(row_to_json(wcm.*)) FROM web_contact_messages wcm WHERE wcm.salon_id = p_salon_id), '[]'::json),
        'analytics', COALESCE((SELECT json_agg(row_to_json(wa.*)) FROM web_analytics wa WHERE wa.salon_id = p_salon_id), '[]'::json)
    ) INTO v_export_data
...
BEGIN
    ...
    WHERE salon_id = p_salon_id AND visit_date < CURRENT_DATE - (p_days_to_keep || ' days')::INTERVAL;
    ...
    WHERE salon_id = p_salon_id AND created_at < NOW() - (p_days_to_keep || ' days')::INTERVAL AND is_replied = true;
    GET DIAGNOSTICS v_deleted_messages = ROW_COUNT;

    -- Elimina prenotazioni vecchie (completate o cancellate)
    DELETE FROM web_bookings 
    WHERE salon_id = p_salon_id AND appointment_date < CURRENT_DATE - (p_days_to_keep || ' days')::INTERVAL AND status IN ('completed', 'cancelled');
...
BEGIN
    ...
    -- Abilita la pagina web
    UPDATE salon_web_settings 
    SET web_enabled = true, updated_at = NOW()
    WHERE salon_id = p_salon_id;

    -- Se non esiste ancora un record, crealo
    IF NOT FOUND THEN
        INSERT INTO salon_web_settings (salon_id, web_enabled, created_at, updated_at)
        VALUES (p_salon_id, true, NOW(), NOW());
    END IF;

    RETURN json_build_object('success', true, 'message', 'Pagina web abilitata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    UPDATE salon_web_settings 
    SET web_enabled = false, updated_at = NOW()
    WHERE salon_id = p_salon_id;
    ...
    RETURN json_build_object('success', true, 'message', 'Pagina web disabilitata con successo');
EXCEPTION WHEN OTHERS THEN
    RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- =====================================================
-- Tutte le funzioni sono state ricreate con la struttura corretta
-- L'errore PostgreSQL 42P13 dovrebbe essere risolto 