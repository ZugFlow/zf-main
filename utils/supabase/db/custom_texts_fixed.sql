...
CREATE INDEX IF NOT EXISTS idx_custom_texts_active ON custom_texts(is_active);
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_modification_success'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_conversion_success'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_archive_success'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_restore_success'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_time_conflict'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_team_member_required'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_service_required'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_date_required'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_time_required'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_customer_name_required'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_phone_required'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_email_invalid'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_phone_invalid'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_past_date'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_outside_hours'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_team_unavailable'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_service_unavailable'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_duplicate'
      AND ct.is_active = true
  );
...
WHERE p.salon_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM custom_texts ct 
    WHERE ct.salon_id = p.salon_id 
      AND ct.text_key = 'booking_system_error'
      AND ct.is_active = true
  );
