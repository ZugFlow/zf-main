...
import { loadSidebarPreference, saveSidebarPreference } from '@/utils/sidebarPreferences';
...
export default function SidebarPage() {
  ...
  const [showChatSection, setShowChatSection] = useState<boolean | null>(null); // null = non ancora caricato
  ...
  const initializeComponent = async () => {
    try {
      ...
      // Load sidebar preference FIRST before setting any UI state
      const sidebarPreference = await loadSidebarPreference();
      if (sidebarPreference !== null) {
        setShowChatSection(sidebarPreference);
        console.log('✅ Loaded sidebar preference:', sidebarPreference ? 'Chat' : 'Appointments');
      } else {
        // Default to appointments if no preference found
        setShowChatSection(false);
        console.log('📋 No sidebar preference found, defaulting to appointments');
      }
      ...
      // On error, default to appointments
      setShowChatSection(false);
      setIsPreferenceLoaded(true);
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const handleToggleChat = () => {
      console.log('🔄 [Sidebar] Chat toggle event received from navbar');
      setShowChatSection(prev => {
        if (prev === null) return false; // If still loading, default to false (appointments)
        const newValue = !prev;
        console.log('🔄 [Sidebar] Toggling chat section:', newValue ? 'Chat' : 'Appointments');
        return newValue;
      });
    };
  ...
  useEffect(() => {
    if (showChatSection === null) return; // Don't emit events while state is loading
  ...
  // Save sidebar preference when showChatSection changes (but only after initial load)
  useEffect(() => {
    if (!isPreferenceLoaded || showChatSection === null) return; // Don't save during initial load or if null
    
    const savePreference = async () => {
      const success = await saveSidebarPreference(showChatSection);
      if (success) {
        console.log('✅ Sidebar preference saved:', showChatSection ? 'Chat' : 'Appointments');
        toast({
          title: t('sidebar.preference_saved'),
          description: `${t('sidebar.sidebar_set_to')} ${showChatSection ? t('sidebar.chat') : t('sidebar.appointments')}`,
          duration: 2000,
        });
      } else {
        console.error('❌ Failed to save sidebar preference');
        toast({
          title: t('sidebar.error_saving_preference'),
          description: t('sidebar.cannot_save_preference'),
          duration: 3000,
          variant: "destructive"
        });
      }
    ...
    savePreference();
  }, [showChatSection, isPreferenceLoaded]);
  ...
  const filteredAppointments = useMemo(() => {
    ...
    return appointments.filter(appointment => {
      ...
      const appointmentDate = startOfDay(new Date(appointment.data));
      ...
      const matchesStatus = selectedStatusFilters.length === 0 
        ? showDeletedAppointments ? true : appointment.status !== 'Eliminato'
  ...
  const handleAppointmentClick = async (appointment: Appointment) => {
    ...
    canEditAppointment(appointment.team_id, hasPermission || (() => false)).then(canEdit => {
      if (canEdit) {
        ...
        toast({
          title: t('sidebar.access_denied'),
          description: t('sidebar.insufficient_permissions'),
          variant: "destructive",
        });
      }
    });
  ...
  const handleDeleteService = async (serviceId: string, orderId: string) => {
    try {
      ...
      toast({
        title: t('sidebar.service_removed'),
        description: t('sidebar.service_removed_description'),
      });
      ...
      toast({
        title: t('sidebar.error_removing_service'),
        description: t('sidebar.cannot_remove_service'),
        variant: "destructive",
      });
    }
  };
  ...
  const handleSaveServices = async (services: Service[], updatedData?: { data?: string; orarioInizio?: string; orarioFine?: string }) => {
    ...
    try {
      ...
      toast({
        title: t('sidebar.services_updated'),
        description: t('sidebar.services_updated_description'),
      });
      ...
      toast({
        title: t('sidebar.error_updating_services'),
        description: t('sidebar.cannot_update_services'),
        variant: "destructive",
      });
    }
  };
  ...
  const AppointmentCard = React.memo(({ appointment }: { appointment: Appointment }) => {
    ...
    const memberName = React.useMemo(() => teamMember?.name || t('sidebar.member'), [teamMember?.name]);
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <span className="text-xs text-gray-600 dark:text-slate-400 font-medium">
                  {getDuration(appointment.orarioInizio, appointment.orarioFine)} {t('sidebar.minutes')}
                </span>
              </div>
            </div>
  ...
  // Show loading while initializing or while showChatSection is not yet determined
  if (isLoading || isInitializing || showChatSection === null) {
    return (
        ...
        <div className="flex items-center justify-center h-full">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">
              {isInitializing ? t('sidebar.loading_preferences') : t('sidebar.loading_data')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-gray-50/50 dark:from-slate-800/30 to-gray-50/30 dark:to-slate-800/10 flex-shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-blue-100 to-blue-200 dark:from-slate-700/60 dark:to-slate-600/60 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm ring-1 ring-blue-100 dark:ring-slate-600/50">
              {showChatSection ? (
                <UserFollow className="h-4 w-4 text-blue-600 dark:text-blue-400" />
              ) : (
                <Calendar className="h-4 w-4 text-blue-600 dark:text-blue-400" />
              )}
            </div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
              {showChatSection ? t('sidebar.chat') : t('sidebar.appointments')}
            </h2>
          ...
          <div className="flex items-center gap-2">
            {/* Toggle button between Chat and Appointments */}
            <Button
              size="sm"
              variant="ghost"
              onClick={() => {
                setShowChatSection(prev => {
                  if (prev === null) return false; // If still loading, default to false
                  const newValue = !prev;
                  console.log('🔄 [Sidebar] Manual toggle:', newValue ? 'Chat' : 'Appointments');
                  return newValue;
                });
              }}
              className="text-muted-foreground hover:text-primary hover:bg-accent p-2 rounded-md transition-all duration-200"
              title={`${showChatSection ? t('sidebar.switch_to_appointments') : t('sidebar.switch_to_chat')}`}
            >
              {showChatSection ? <Calendar className="h-4 w-4" /> : <UserFollow className="h-4 w-4" />}
            </Button>
            {!showChatSection && (
              <Button
                size="sm"
                ...
                onClick={() => {
                  if (salonId) {
                    ...
                    fetchCustomStatuses();
                    toast({
                      title: t('sidebar.manual_refresh'),
                      description: t('sidebar.data_updated_manually'),
                      duration: 1500,
                    });
                  }
                }}
        ...
        {!showChatSection && (
          <>
            <div className="relative mb-3">
              ...
              <Input
                placeholder={t('sidebar.search_appointments')}
                value={searchTerm}
          ...
          {showFilters && (
            ...
            >
              <div className="p-4 space-y-4">
                ...
                <div>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent className="max-h-60 overflow-y-auto">
                      <DropdownMenuLabel>{t('sidebar.status')}</DropdownMenuLabel>
                ...
                {/* Member Filters */}
                <div>
                  <h4 className="text-sm font-semibold text-foreground mb-2">{t('sidebar.members')}</h4>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-foreground">{t('sidebar.show_deleted')}</span>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  >
                    {showDeletedAppointments ? t('sidebar.hide') : t('sidebar.show')}
                  </Button>
                </div>
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100 mb-2">
                {t('sidebar.no_appointments')}
                  ...
                  {searchTerm || selectedStatusFilters.length > 0 || selectedMembers.length > 0 || showDeletedAppointments
                    ? t('sidebar.no_appointments_filter')
                    : appointments.length === 0 
                      ? t('sidebar.no_appointments_system')
                      : t('sidebar.no_appointments_selected')
                  }
                </p>
